...
type Chapter = { id:string; kicker:string; brand:string; title:string; copy:string; meta:string; color:string; model?:keyof typeof MODELS; route?:string };
...
function useStoryProgress(ref:React.RefObject<HTMLElement>){
 ...
 useEffect(()=>{
  const read=()=>{const el=ref.current;if(!el)return;const top=window.scrollY+el.getBoundingClientRect().top;const max=Math.max(1,el.offsetHeight-window.innerHeight);setTarget(clamp((window.scrollY-top)/max))};
  const onScroll=()=>{if(!raf.current)raf.current=requestAnimationFrame(()=>{raf.current=0;read()})};
  read();window.addEventListener("scroll",onScroll,{passive:true});window.addEventListener("resize",read);
  return()=>{window.removeEventListener("scroll",onScroll);window.removeEventListener("resize",read);if(raf.current)cancelAnimationFrame(raf.current)};
 },[ref]);
...
function ScrollFilm({progress}:{progress:number}){
 const video=useRef<HTMLVideoElement>(null);const current=useRef(0);const [ready,setReady]=useState(false);const isMobile=typeof window!=="undefined"&&window.innerWidth<768;
 useEffect(()=>{const el=video.current;if(!el)return;const onMeta=()=>setReady(true);el.addEventListener("loadedmetadata",onMeta);el.preload="auto";el.load();return()=>el.removeEventListener("loadedmetadata",onMeta)},[]);
 useEffect(()=>{const el=video.current;if(!el||!ready||!Number.isFinite(el.duration)||el.duration<=0)return;const target=progress*Math.max(0,el.duration-.05);current.current+=(target-current.current)*.12;if(Math.abs(el.currentTime-current.current)>.035){try{el.currentTime=current.current}catch{/* media is still buffering */}}},[progress,ready]);
 return <div className="pointer-events-none absolute inset-0 overflow-hidden bg-black">
  <video ref={video} src={isMobile?VIDEO_MOBILE:VIDEO_DESKTOP} poster={POSTER} muted playsInline preload="auto" className="h-full w-full object-cover scale-[1.015]" />
...
function PointerRig({pointer}:{pointer:React.MutableRefObject<THREE.Vector2>}){useEffect(()=>{const move=(e:PointerEvent)=>pointer.current.set((e.clientX/innerWidth)*2-1,(e.clientY/innerHeight)*2-1);addEventListener("pointermove",move,{passive:true});return()=>removeEventListener("pointermove",move)},[pointer]);return null}

function Origin({progress,pointer}:{progress:number;pointer:React.MutableRefObject<THREE.Vector2>}){
 const core=useRef<THREE.Mesh>(null),group=useRef<THREE.Group>(null);
 useFrame(({clock})=>{if(!group.current||!core.current)return;const t=clock.elapsedTime;group.current.rotation.y+=.0018;group.current.rotation.x+=(pointer.current.y*.12-group.current.rotation.x)*.025;core.current.scale.setScalar(.2+ease(progress)*1.9+Math.sin(t*1.5)*.025)});
