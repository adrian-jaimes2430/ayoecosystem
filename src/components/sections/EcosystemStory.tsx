...
import { ArrowDown, ArrowUpRight, Volume2 } from "lucide-react";
...
function useStoryProgress(ref: React.RefObject<HTMLElement>) {
  ...
  useEffect(() => {
    ...
    const onScroll = () => requestAnimationFrame(read);
    read();
  ...
  return <section id="historia" ref={ref} className="relative min-h-[900svh] overflow-hidden bg-black text-white">
    <div className="sticky top-0 h-[100svh] overflow-hidden">
      ...
      >
        ...
        <StoryWorld chapter={chapter} local={local} pointer={pointer} navigate={navigate} />
      </Canvas>

      <div className="absolute inset-x-0 top-0 z-10 flex items-start justify-between p-5 md:p-8 pointer-events-none">
        <div className="text-[9px] uppercase tracking-[0.4em] text-white/45">A&O ECOSYSTEM · HISTORY</div>
        <div className="text-[9px] uppercase tracking-[0.32em] text-white/45">{chapter.id} / 08</div>
      </div>
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 px-6 pb-8 md:px-10 md:pb-10 pointer-events-none">
        <div className="mx-auto flex max-w-7xl items-end justify-between gap-8">
          <div className="max-w-2xl" style={{ opacity: 0.55 + ease(local) * 0.45 }}>
            ...
            <h2 className="mt-3 max-w-3xl font-display text-3xl font-semibold leading-[0.98] tracking-tight sm:text-5xl md:text-6xl">{chapter.title}</h2>
            <p className="mt-4 max-w-xl text-sm leading-relaxed text-white/62 md:text-base">{chapter.copy}</p>
            <div className="mt-5 flex flex-wrap items-center gap-3 text-[9px] uppercase tracking-[0.24em] text-white/45">
          ...
          <div className="hidden md:flex flex-col items-end gap-3 text-right text-[9px] uppercase tracking-[0.25em] text-white/40">
      ...
      {chapter.route && <button type="button" onClick={() => navigate(chapter.route!)} className="absolute right-6 top-1/2 z-20 hidden -translate-y-1/2 items-center gap-2 rounded-full border border-white/20 bg-black/20 px-4 py-2 text-[9px] uppercase tracking-[0.25em] text-white/75 backdrop-blur-md transition hover:border-white/50 hover:bg-white/10 md:flex">
        Entrar <ArrowUpRight className="h-3.5 w-3.5" />
      </button>}

      <div className="absolute bottom-5 left-1/2 z-20 -translate-x-1/2 text-white/35 pointer-events-none">
