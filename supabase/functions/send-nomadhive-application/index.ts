...
Deno.serve(async (req) => {
  ...
  if (!allowRequest(`nomadhive:${clientKey(req)}`)) {
  ...
  const { name, email, phone, experience } = parsed.data
  const result = await notifyTeam(
    'nomadhive-application',
    {
      name,
      email,
      phone,
    ...
    `nomadhive-app-${email}-${new Date().toISOString().slice(0, 13)}`,
  )
